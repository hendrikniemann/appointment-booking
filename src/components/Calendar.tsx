...
export default function Calendar(props: CalendarProps) {
  ...
  const appointments = useQuery(
    ["fetchWeek", currentWeek.year + "-" + currentWeek.week],
    () =>
      getSlotsInInterval(props.modelId, {
  ...
  return (
    <Box>
      ...
      <VStack py="4" height="275px" overflowY="auto">
        {appointments.isFetching ? (
          <Center h="100%">
            <Spinner />
          </Center>
        ) : (
          appointments.data?.model?.slots.edges?.map((edge) => {
            if (!edge || !edge.node) {
              return null;
            }
